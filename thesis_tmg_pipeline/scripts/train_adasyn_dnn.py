...
def train(
    ...
    train_dl = DataLoader(adasyn["train_dataset"], shuffle=True, **dl_kwargs)
    ...
    print(
        f"Resampling: {adasyn['sampling_method']} | "
        f"class counts before={adasyn['class_counts_before']} after={adasyn['class_counts_after']}"
    )
        ...
        if should_save:
            manager.save(
                payload={
                    ...
                    "config": vars(config),
                    "adasyn": {
                        "sampling_method": adasyn["sampling_method"],
                        "effective_neighbors": adasyn["effective_neighbors"],
                        "class_counts_before": adasyn["class_counts_before"],
                        "class_counts_after": adasyn["class_counts_after"],
                    },
                },
                epoch=epoch + 1,
                is_best=is_best,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train resumable ADASYN+DNN baseline for CICIDS2017/UNSW-NB15")
    ...
    parser.add_argument("--run-name", type=str, default="adasyn_dnn")
    ...
    parser.add_argument("--adasyn-strategy", type=str, default="not majority")
...
if __name__ == "__main__":
    ...
    train(
        ...
        adasyn_strategy=args.adasyn_strategy,
    )
