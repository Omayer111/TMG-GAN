...
def build_augmented_dataset(
    generator: TACGANTabularGenerator,
...
def train(
    ...
    discriminator = TACGANTabularDiscriminator(
        feature_dim=data["input_dim"],
        num_classes=data["num_classes"],
        hidden_dim=gan_hidden_dim,
    ).to(device)
    ...
    if phase == "gan" and gan_start_epoch < gan_epochs:
        for gan_epoch in range(gan_start_epoch, gan_epochs):
            ...
            for real_x, real_y in gan_train_dl:
                ...
                ones = torch.ones(len(real_y), device=device)
                zeros = torch.zeros(len(real_y), device=device)

                for _ in range(d_steps):
                    d_optimizer.zero_grad(set_to_none=True)
                    with torch.autocast(device_type=device.type, enabled=config.use_amp and device.type == "cuda"):
                        src_real, cls_real = discriminator(real_x)
                        loss_src_real = bce(src_real, ones)
                        loss_cls_real = ce(cls_real, real_y)

                        fake_x = generator.sample(real_y, device=device).detach()
                        src_fake, cls_fake = discriminator(fake_x)
                        loss_src_fake = bce(src_fake, zeros)
                        loss_cls_fake = ce(cls_fake, real_y)

                        d_loss = loss_src_real + loss_src_fake + lambda_cls * 0.5 * (loss_cls_real + loss_cls_fake)
                ...
                for _ in range(g_steps):
                    ...
                    with torch.autocast(device_type=device.type, enabled=config.use_amp and device.type == "cuda"):
                        ...
                        src_fake, cls_fake = discriminator(fake_x)
                        g_adv_loss = bce(src_fake, ones)
                        g_cls_loss = ce(cls_fake, real_y)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train resumable tabular TACGAN-IDS baseline for CICIDS2017/UNSW-NB15")
    ...
    parser.add_argument("--run-name", type=str, default="tacgan_ids")
